...
//Security options
session_start();
if (isset($_GET["editMode"]) && $_GET["editMode"] == "4t74ygv6") $_SESSION["allowEdit"] = true; else $_SESSION["allowEdit"] = false;
$allow_delete = $_SESSION["allowEdit"];// Set to false to disable delete button and delete POST request.
$allow_upload = $_SESSION["allowEdit"]; // Set to true to allow upload files
...
$hidden_extensions = ['php','h','json']; // must be an array of lowercase file extensions. Extensions hidden in directory index
...
{ shell_exec("yt-dlp '{$_POST["youtube_url"]}'"); die("\nDownloaded"); }
...
function rmrf($dir) {
	if(is_dir($dir)) {
		$files = array_diff(scandir($dir), ['.','..','.private']);
